...
<div class="search-bar">
    ...
    {#each tags as tag}
        ...
        <div class="search-item">
            <a data-sveltekit-reload href={"/projects"+queryPopTag(tag)}><i class="fa-solid fa-x"></i></a>
            <div>tag:{tag}</div>
        </div>
        {/if}
    {/each}
    {#if search}
        <div class="search-item">
            <a data-sveltekit-reload href={"/projects"+queryPopSearch()}><i class="fa-solid fa-x"></i></a>
            <div>"{search}"</div>
        </div>
    {/if}
    </div>
    {#if search || (tags.length>0)}
        <p>{data.projectCount} results</p>
    {/if}
    <div class="input">
        <form action="/projects" data-sveltekit-reload>
